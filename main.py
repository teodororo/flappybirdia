...
def game_start(generation,x,y):
	...
	while(True):
		...
		if crashTest:
			reward = -1000 # em caso de batida
			Q_update(x_prev,y_prev,jump,reward,x_new,y_new)
			return score

		reward = 15 # caso consiga passar 
...
def convert(birdxpos,birdypos,bttm_pipes):
	...
	y = bttm_pipes[0]['y']-birdypos # distancia para o tubo 
	if(y<0):
		y=abs(y)+408 
...
def Q_update(x_prev,y_prev,jump,reward,x_new,y_new):
	print("x: ",x_new,", y: ",y_new,"reforço: ",reward)
	if jump:
...
if __name__=="__main__":
	...
	pygame.display.set_caption("PROJETO DE IA")
